...
const buyStock = async (req, res) => {
    try {
        ...
        if (!price || price <= 0) {
            return res.status(404).json({ success: false, message: `Stock price not available for: ${symbol}` });
        }

        // Save transaction
        await InvestmentTransaction.create({ userId, symbol, quantity, price, type: 'BUY' });
        ...
        if (existing) {
            ...
            await Investment.create({ userId, symbol, quantity, avgPrice: price });
        }
...
const sellStock = async (req, res) => {
    try {
        ...
        if (!inv) {
            return res.status(404).json({ success: false, message: `Investment not found for symbol: ${symbol}` });
        }

        if (inv.quantity < quantity) {
            return res.status(403).json({ success: false, message: 'Not enough quantity to sell' });
        }
        ...
        // Save transaction
        await InvestmentTransaction.create({ userId, symbol, quantity, price, type: 'SELL' });
    ...
    try {
        ...
        const portfolio = await Promise.all(
            investments.map(async (inv) => {
                ...
                const profitPercent = investedValue === 0 ? 0 : (profit / investedValue) * 100;
...
const getPortfolioSummary = async (req, res) => {
    try {
        ...
        const profitPercent = totalInvested === 0 ? 0 : (totalProfit / totalInvested) * 100;

        return res.status(200).json({
            success: true,
            data: { totalInvested, totalCurrentValue, totalProfit, profitPercent },
        });
    } catch (err) {
...
const getTransactions = async (req, res) => {
    try {
        const userId = req.userId;
        const transactions = await InvestmentTransaction.find({ userId }).sort({ createdAt: -1 });
...
module.exports = { buyStock, sellStock, getPortfolio, getPortfolioSummary, getAllInvestments, getTransactions };