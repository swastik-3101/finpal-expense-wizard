...
exports.getCategories = async (req, res) => {
  try {
    ...
    if (categories.length === 0) {
      return res.json(['Food', 'Transportation', 'Housing', 'Utilities', 'Entertainment', 'Healthcare', 'Shopping', 'Other']);
    }
    res.json(categories);
  } catch (err) {
    console.error(err.message);
...
exports.uploadReceipt = async (req, res) => {
  try {
    ...
    formData.append('receipt', fs.createReadStream(req.file.path), req.file.originalname);
    ...
    console.error(err.message);
    res.status(500).json({ msg: 'Error processing receipt' });
  }
};
